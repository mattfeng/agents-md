from __future__ import annotations

import argparse
import sys
from importlib import resources
from pathlib import Path


START_MARKER = "<!-- agents-md-templates:start -->"
END_MARKER = "<!-- agents-md-templates:end -->"
TEMPLATE_PREFIX = "AGENTS."
TEMPLATE_SUFFIX = ".md"
BASE_TEMPLATE = "base"


def main(argv: list[str] | None = None) -> int:
    parser = build_parser()
    args = parser.parse_args(argv)
    return args.func(args)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="agents-md",
        description="Generate and update AGENTS.md files from bundled templates.",
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    write_parser = subparsers.add_parser(
        "write",
        help="write or update an AGENTS.md file",
    )
    write_parser.add_argument(
        "--target",
        type=Path,
        default=Path.cwd(),
        help="project directory or AGENTS.md path to write (default: current directory)",
    )
    write_parser.add_argument(
        "--template",
        action="append",
        choices=template_names(),
        help="template to include; repeat for multiple templates (skips the prompt)",
    )
    write_parser.add_argument(
        "--replace",
        action="store_true",
        help="replace the entire AGENTS.md file instead of updating the managed block",
    )
    write_parser.set_defaults(func=write_command)

    list_parser = subparsers.add_parser("list", help="list bundled templates")
    list_parser.set_defaults(func=list_command)

    return parser


def list_command(_args: argparse.Namespace) -> int:
    for name in template_names():
        print(name)
    return 0


def write_command(args: argparse.Namespace) -> int:
    target = args.target.resolve()
    output_path = target if target.name == "AGENTS.md" else target / "AGENTS.md"
    project_dir = output_path.parent

    selected = select_templates(explicit=args.template)
    generated = render_managed_block(selected)

    if args.replace:
        next_content = generated + "\n"
    else:
        current = output_path.read_text(encoding="utf-8") if output_path.exists() else ""
        next_content = upsert_managed_block(current, generated)

    print(next_content, end="")
    sys.stdout.flush()

    project_dir.mkdir(parents=True, exist_ok=True)
    output_path.write_text(next_content, encoding="utf-8")
    print(f"\nWrote {output_path} with templates: {', '.join(selected)}", file=sys.stderr)
    return 0


def select_templates(*, explicit: list[str] | None) -> list[str]:
    selected: list[str] = []

    def add(name: str) -> None:
        if name not in selected:
            selected.append(name)

    if explicit:
        for name in explicit:
            add(name)

    if not explicit:
        selected = prompt_for_templates()

    if not selected:
        add(BASE_TEMPLATE)

    return selected


def prompt_for_templates() -> list[str]:
    selected: list[str] = []

    print("Select templates to include in AGENTS.md:", file=sys.stderr)
    for name in template_names():
        default = name == BASE_TEMPLATE
        if confirm(f"Include {name}?", default=default):
            selected.append(name)

    return selected


def confirm(prompt: str, *, default: bool) -> bool:
    suffix = " [Y/n] " if default else " [y/N] "

    while True:
        print(prompt + suffix, end="", file=sys.stderr, flush=True)
        answer = sys.stdin.readline().strip().lower()
        if not answer:
            return default
        if answer in {"y", "yes"}:
            return True
        if answer in {"n", "no"}:
            return False
        print("Please answer y or n.", file=sys.stderr)


def render_managed_block(selected: list[str]) -> str:
    chunks = [
        START_MARKER,
        "<!-- Generated by `agents-md write`; edit templates or content outside this block. -->",
    ]
    chunks.extend(render_template(name) for name in selected)
    chunks.append(END_MARKER)
    return "\n\n".join(chunks).strip()


def render_template(name: str) -> str:
    return template_path(name).read_text(encoding="utf-8").strip()


def upsert_managed_block(current: str, generated: str) -> str:
    current = current.rstrip()
    start = current.find(START_MARKER)
    end = current.find(END_MARKER)

    if start != -1 and end != -1 and start < end:
        end += len(END_MARKER)
        return f"{current[:start].rstrip()}\n\n{generated}\n\n{current[end:].lstrip()}".strip() + "\n"

    if not current:
        return generated + "\n"

    return f"{current}\n\n{generated}\n"


def template_names() -> list[str]:
    names = []
    for path in templates_dir().iterdir():
        if path.name == "AGENTS.md":
            names.append(BASE_TEMPLATE)
        elif path.name.startswith(TEMPLATE_PREFIX) and path.name.endswith(TEMPLATE_SUFFIX):
            names.append(path.name.removeprefix(TEMPLATE_PREFIX).removesuffix(TEMPLATE_SUFFIX))
    return sorted(names)


def template_path(name: str) -> Path:
    filename = "AGENTS.md" if name == BASE_TEMPLATE else f"{TEMPLATE_PREFIX}{name}{TEMPLATE_SUFFIX}"
    path = templates_dir() / filename
    if not path.exists():
        available = ", ".join(template_names())
        raise SystemExit(f"unknown template {name!r}; available templates: {available}")
    return path


def templates_dir() -> Path:
    return Path(str(resources.files("agents_md_templates") / "templates"))


if __name__ == "__main__":
    raise SystemExit(main(sys.argv[1:]))
